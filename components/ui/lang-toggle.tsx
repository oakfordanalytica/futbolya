...
type LangToggleProps =
  | { showText?: true; ariaLabel?: string }
  | { showText: false; ariaLabel: string };

const LOCALES = ["en", "es"] as const;

export function LangToggle({ showText = true, ariaLabel }: LangToggleProps) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <Languages className="h-4 w-4" />
          {showText && <span className="font-medium">{currentLangLabel}</span>}
          {/* {showText && <ChevronsUpDown className="ml-auto h-4 w-4" />} */}
        </Button>
      </DropdownMenuTrigger>
